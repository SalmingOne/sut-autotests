...
class TestLaborCostPage:
    ...
    def test_edit_overwork_obligatory_reason(self, second_project, f_overtime_reason_requirement, login, driver):
        ...
        labor_cost_page.add_overtime_work_without_file(today[0]-1, 2, 'SecondProject')
        ...
        project_day_cell_contents_1 = labor_cost_page.get_project_day_cell_contents("SecondProject", today[0]+1)
        total_column_1 = labor_cost_page.get_project_total("SecondProject")
        in_total_row_1 = labor_cost_page.get_day_total_raw(today[0])
        # -1 иначе выбирает завтрашний день. Хотя при выводе print(today[0]) число верное
        labor_cost_page.open_overwork_drover_for_specific_day(today[0]-1, 'SecondProject')
        labor_cost_page.check_data_from_drover(2, "Много работал")
        labor_cost_page.editing_overwork(5, 'Еще больше работал')
        labor_cost_page.submit_labor_reason()
        time.sleep(1)
        labor_cost_page.save_labor_reason()
        project_day_cell_contents_2 = labor_cost_page.get_project_day_cell_contents("SecondProject", today[0]+1)
        total_column_2 = labor_cost_page.get_project_total("SecondProject")
        in_total_row_2 = labor_cost_page.get_day_total_raw(today[0])
        assert project_day_cell_contents_2 != project_day_cell_contents_1, 'Часы переработки неверные'
        assert total_column_2 != total_column_1, 'Часы переработки неверные'
        assert in_total_row_2 != in_total_row_1, 'Часы переработки неверные'